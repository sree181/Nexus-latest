...
- **Developer session workspace:** The Developer role lands on live connected-agent Sessions rather than the demo task runner. Session detail routes expose ordered Activity and package-policy Security evidence, projection health, UTC timestamps, and deep links into the associated governed HyperMesh run. The demo task runner remains a secondary development utility.
