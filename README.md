...
- **CISO governance contracts:** Policies now retain immutable versions with draft, review, active, superseded, and retired states; effective intervals; content digests; and append-only events. Exception requests bind the active policy version, its digest, the submitted evidence, owner, controls, and expiry into one request digest. Approval, exception state, and decision event commit atomically. Priority 5A intentionally leaves policy maker-checker enforcement, materialized expiry, HyperMesh governance projection, and the redesigned CISO workspace to the next ordered stages. See the [Priority 5A Technical Design](docs/PRIORITY5A_GOVERNANCE_CONTRACTS.md) and [Priority 5 Project Plan](docs/PRIORITY5_PROJECT_PLAN.md).
