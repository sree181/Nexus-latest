...
| Weather | NOAA National Weather Service `api.weather.gov` | Free public API, no key and no agreement. Lee County is county zone `ALC081` and forecast zone `ALZ047`; Jordan-Hare falls in grid `BMX 111,47`.[11] | Ingest active watches, warnings, and advisories naming a Lee County zone, plus the 12-hour gridded forecast as operating context. Key alerts by VTEC event tracking number so a warning keeps one identity across its updates. Drop the `Test` keepalive product. Nexus reproduces NWS products and never issues, amends, or cancels a public alert. |
| Stream and seismic hazard | USGS NWIS instantaneous values and FDSN earthquake catalog | Free public APIs, no key. Lee County has two active gauges: Sougahatchee Creek at CR 188 (`02418230`) and Chewacla Creek at Chewacla State Park (`02418760`).[12] | Ingest gage height, discharge, and the stage change the gauge itself recorded, plus regional earthquakes within 300 km. USGS publishes no flood stage for these creeks, so Nexus reports measured movement and asks for a crossing inspection; it never declares a road flooded. |
...
[11]: https://api.weather.gov/points/32.6025,-85.4900 "NWS gridpoint for Jordan-Hare Stadium"
[12]: https://waterservices.usgs.gov/nwis/iv/?format=json&countyCd=01081&parameterCd=00060,00065&siteStatus=active "USGS instantaneous values, Lee County Alabama"
