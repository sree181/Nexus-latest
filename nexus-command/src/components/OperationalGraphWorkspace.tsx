...
export function OperationalGraphWorkspace() {
  ...
  return <main className="graph-workspace">
    <header className="graph-header"><div><span>LIVE OPERATIONAL GRAPH</span><h1>{viewLabels[view].title}</h1><p>{viewLabels[view].subtitle}</p></div><div className="graph-header__status"><strong>{event?.name || 'Loading active event'}</strong><span>{snapshot ? `As of ${new Date(snapshot.asOf).toLocaleTimeString()}` : 'Awaiting graph state'}</span></div></header>
    ...
    <footer className="graph-timeline"><span>STATE HISTORY</span><div><i /><strong>Current authoritative state</strong><small>{snapshot ? new Date(snapshot.generatedAt).toLocaleString() : 'Not loaded'}</small></div><button disabled={!selected}>Open selected entity history</button></footer>
  </main>;
}
