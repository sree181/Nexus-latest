...
| Governance lifecycle        | Track `last_completed_at`, `last_error`, expiry transition counts, pending approvals past their deadline, expiring exceptions, notification backlog, stale-version conflicts, and maker-checker denials | Alert if reconciliation has no successful completion for more than three configured intervals or reports an error. Investigate any due row that remains pending/approved after the next scan. |
