...
| Projection recovery | Keep the default 5-second reconciliation scan and 1-second retry base unless load testing supports a change; override with `MESHAGENT_PROJECTION_RETRY_SECONDS` and `MESHAGENT_PROJECTION_RETRY_BASE_SECONDS` only through deployment configuration | Alert on repeated failed Developer activity projection, failed `review_projection_outbox` rows, increasing retry attempts, and adapter `queue_rejected_events`. Review workflow events and their projection work are committed in one control-plane transaction. Retry backoff is capped at five minutes; relational ledgers remain the delivery/workflow authority while HyperMesh is unavailable. |
| Analyst operations  | Upgrade with an application-consistent backup of the complete state directory; allow the control-plane initializer to add review assignment fields and the comments, saved-view, notification, and read-state tables | Validate Analyst/CISO access, Developer denial, stale-version rejection, partial-success bulk receipts, personal-view isolation, notification read isolation, and review-to-case evidence links. The v1 unified queue reads a bounded working set of 100 cases and 500 reviews before filtering; alert before inventory approaches that boundary. |
| CISO governance contracts | Back up the complete state directory before Priority 5A. Startup adds policy/exception lifecycle columns and immutable event tables, then deterministically backfills digests, effective intervals, names, and events | Reopen the migrated store twice and confirm one active policy version, stable version states and digests, and no duplicate events. Exercise stale policy and approval versions, exact exception policy binding, duplicate-scope rejection, and requester/approver separation. |
| Governance enforcement | Keep one API writer per state directory. The API runs expiry reconciliation at startup and every 60 seconds by default; set `MESHAGENT_GOVERNANCE_RECONCILE_SECONDS` only after load and failure testing | `GET /api/governance/lifecycle/status` must show a recent completion and no error. In production, verify two distinct CISO identities for policy activation. Exercise requester separation, renewal lineage, revocation, exact-boundary expiry, notification isolation, and restart idempotency on a state copy before rollout. |
| Governance evidence | Back up the control plane and every HyperMesh sibling store together. Policy and exception events commit with `governance_projection_outbox`; the projection worker validates the canonical payload and immutable source digests before writing the dedicated governance store | Alert on pending or failed projection rows that do not converge. Validate `GET /api/policies/{id}/evidence` and `GET /api/exceptions/{id}/evidence`, confirm Developer denial, and restart on an isolated state copy to prove native ULIDs and relation counts remain unchanged. Do not present a partial or sample-mode graph as governed evidence. |
| CISO operating workspace | Keep policy, approval, exception, remediation, report, and native evidence endpoints behind their existing API capabilities; the frontend must not become an authorization layer | Walk Command center → Policy control → Decision desk → Exception record → Remediation portfolio → Assurance reports. Confirm every metric opens a source record, unknown coverage remains explicit, a pending exception points to the decision desk, stale remediation commands return `412`, and verified or exception-covered outcomes require evidence. |
...
4. With two CISO identities, submit and independently activate a policy revision. Request an exception as an Analyst, decide it as a CISO, request renewal through a different authorized identity, revoke it independently, and confirm the requester is blocked from approval, renewal, and revocation. Check `GET /api/governance/lifecycle/status`, then verify a due approval and approved exception materialize as expired without duplicate events after restart.
5. Retrieve native evidence for the policy and exception. Record the returned projection digests and native ULIDs, restart the isolated candidate, and confirm the values and scoped relation counts are unchanged. Confirm a Developer receives `403` from both governance evidence endpoints.
6. As a CISO, open every governance workspace, follow a waiting decision to its exact exception and policy version, and create remediation against a non-production case. Confirm the API rejects terminal completion without evidence, rejects a stale aggregate version with `412`, records verified evidence, and preserves the immutable event sequence after restart.
7. As an Analyst, assign a test review with a due time, add a note, save a personal view, and confirm the notification and Work History entry. Confirm a CISO can open the unified queue and a Developer receives `403` from the operations API. Exercise a partial-success bulk assignment in non-production and retain the item-level receipt.
8. Pair a dedicated test device, record a benign event, then revoke it and confirm a subsequent recorder request is rejected.
9. Create an encrypted backup, verify it, and complete the disaster-recovery drill procedure before accepting a new environment.
...
| Analyst work operations     | Track open, unassigned, and overdue work; stale assignment conflicts; bulk item failures; notification backlog; control-plane database growth; and failed review projections without exporting note text | Establish queue-volume and age thresholds per deployment. Add database-level pagination before the bounded 100-case/500-review working set becomes insufficient. |
| Governance lifecycle        | Track `last_completed_at`, `last_error`, expiry transition counts, pending approvals past their deadline, expiring exceptions, notification backlog, stale-version conflicts, maker-checker denials, and governance projection status/attempts | Alert if reconciliation has no successful completion for more than three configured intervals or reports an error. Investigate any due row that remains pending/approved after the next scan and any projection backlog that does not converge. |
