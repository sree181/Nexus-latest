...
| Model integration | Keep `OPENAI_API_KEY` and optional endpoint configuration in a secret manager; approve outbound network destinations and data handling before enabling it | No model key means new runs record the reference build rather than an arbitrary external model execution. |
