...
| Projection recovery | Keep the default 5-second reconciliation scan and 1-second retry base unless load testing supports a change; override with `MESHAGENT_PROJECTION_RETRY_SECONDS` and `MESHAGENT_PROJECTION_RETRY_BASE_SECONDS` only through deployment configuration | Alert on repeated `failed` projection status, increasing retry attempts, and adapter `queue_rejected_events`. Retry backoff is capped at five minutes; the activity ledger remains the delivery authority while HyperMesh is unavailable. |
