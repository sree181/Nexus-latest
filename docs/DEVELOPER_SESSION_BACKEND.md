...
The production web application now uses this API as a visual-first Developer landing experience. A repository-derived **Project** selector scopes connected work without inventing a second workspace entity. **Sessions** lists the current developer's Cursor and Claude Code work, status, last activity, and evidence state. A selected session has stable **Overview**, **Activity**, **Security**, and **Evidence** tabs. Activity preserves server order while moving payloads, retry timestamps, identifiers, and digests into an on-demand drawer. Security presents policy decisions, requested and installed package milestones, advisory availability, and evidence sync as separate facts.

Global Developer navigation contains only **Sessions**, **Attention**, and **Connections**. Connections groups editor setup, repository opt-in and privacy configuration, paired devices, and API-backed verification; it is intentionally a control plane rather than an IDE. The legacy task runner remains available only behind a development-tools menu. Analyst and CISO routes remain capability guarded. Active screens use bounded polling against the owner-scoped read APIs. A future increment may replace polling with a server stream and add server-side cursors for inventories beyond the current bounded pages.

## 11. Developer Attention and Security review

The global Developer navigation now includes **Attention** between Sessions and Connections. It is derived from the caller's own package-policy evaluations and contains blocked checks, incomplete checks, or checks with published advisories. Package recognition is grammar based: the Cursor and Claude Code adapters recognize supported `pip`, `uv`, `poetry`, `npm`, and `yarn` install forms at shell boundaries. Navigation and inspection commands such as `cd`, `head`, `tail`, `git show`, `pip show`, and `npm view` are not package events. Failed installs never become installed-package evidence.
...
A Developer may create one durable review request per policy evaluation for a safe version, security guidance, temporary approval, or false-positive review. The request freezes the session, repository, package/version, ecosystem, CVEs, reasons, and proven code links. Analysts receive a server-prioritized queue and can request changes, reject, mark a false positive, approve a time-bounded exception, or escalate. The Developer cannot decide their own request; updates use optimistic concurrency and append immutable review events. When a later package-gate check for the same session and package returns clean, an outstanding change request is automatically marked **verified** with the gate evidence identifier.
