...
Global Developer navigation contains only **Sessions** and **Connections**. Connections groups editor setup, repository opt-in and privacy configuration, paired devices, and API-backed verification; it is intentionally a control plane rather than an IDE. The legacy task runner remains available only behind a development-tools menu. Analyst and CISO routes remain capability guarded and unchanged. Active screens use bounded polling against the owner-scoped read APIs. A future increment may replace polling with a server stream and add server-side cursors for inventories beyond the current bounded pages.
