"""
Patent Forward Star (Impact) Strategy
========================================
Creates star-shaped hyperedges where the primary patent is always
included alongside every patent that has cited it (forward citations).

Model
-----
For each primary patent P with forward citations [F1, F2, ..., Fk]:
  → hyperedge members = {P, F1, F2, ..., Fk}
  → formation = FORWARD_STAR
  → weight    = k / max_k   (normalised forward-citation impact)
  → event_ts  = P's publication date

Why this model?
  Forward citation count is the most widely used patent impact metric
  ("how often is this patent cited by later work?").
  In hypergraph form this creates a focal-node structure that graph
  analytics (PageRank, s-betweenness) can directly interpret as
  influence topology.
"""
...
class PatentForwardStarStrategy(IngestStrategy):
    name        = "patent_forward_star"
    label       = "Forward Citation Impact Stars"
    description = (
        "Builds focal-star hyperedges of primary patent + all patents that "
        "cite it, surfacing high-impact patents via PageRank and s-betweenness."
    )
    category    = "patent"
    param_specs = [
        ParamSpec(
        ...
        ParamSpec(
            ...
            type="string", default="PATENT_FORWARD_STAR",
            required=True,
        ),
        ParamSpec(
            name="min_citations", label="Minimum Forward Citations",
            type="integer", default=1, min=1,
            description="Skip patents with fewer forward citations than this.",
        ),
        ParamSpec(
    ...
    def run(
        ...
        table         = config.get("target_table", "PATENT_FORWARD_STAR").upper()
        min_citations = int(config.get("min_citations", 1))
        ...
        notes.append(f"Impact stars: {len(raw_hedges)}, entities: {len(entity_to_id)}")
        ...
        progress_cb(0.40, f"Inserting {len(all_hedges)} impact star hyperedges …")
